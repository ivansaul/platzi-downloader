...
def _extract_streaming_urls(content: str) -> list[str] | None:
    ...
    urls = [] # Se guarda las resoluciones del video
...
async def _worker_ts_dl(urls: list, dir: Path, **kwargs):
    ...
    # --- NEW FEATURES: progress bar format ---
...
async def m3u8_dl(
    ...
    **kwargs,
    ...
    # --- NEW FEATURES: implement quality selection ---
    ...
    try:
        ...
        m3u8_urls = _extract_streaming_urls(await response.text()) # El enlace .m3u8 contiene las resoluciones del video.
 
        if not m3u8_urls:
            raise Exception("No m3u8 urls found")

        await _m3u8_dl(m3u8_urls[int(quality)], path, **kwargs) # Aquí va la resolucion del video [0]=1280; [1]=1920
